...
const FiltersControls = ({
  ...
  return (
    <div className={containerClass}>
      ...
      <DifficultyFilter difficultyFilter={difficultyFilter} onChangeDifficulty={onChangeDifficulty} variant={variant} />

      <div className="grid grid-cols-1 gap-2 overflow-visible">
        <DropdownFilter label="Topic" value={topicTagFilter} onChange={onChangeTopic} options={allTags} />
        <DropdownFilter label="Company" value={companyFilter} onChange={onChangeCompany} options={allCompanies} />
      </div>

      {(topicTagFilter || companyFilter) && (
        <Button variant="destructive" size="sm" onClick={onClear} className="text-xs">Clear filters</Button>
      )}
    </div>
  );
};
